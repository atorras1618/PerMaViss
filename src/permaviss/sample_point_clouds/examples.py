...
def circle(no_points, radius):
    ...
    for n in range(no_points):
        point_cloud.append([radius * np.cos(n * step), radius * np.sin(n * step)])
...
def cube_four(div):
    ...
    for i in range(div):
        for j in range(div):
            for k in range(div):
                for l in range(div):
                    point_cloud.append([i-shift,j-shift,k-shift,l-shift])
...
def torus(div, min_rad,max_rad):
    ...
    while(ang_max < 2 * np.pi):
        ...
        while(ang_min < 2 * np.pi):
            ...
            point_cloud.append([min_rad * np.cos(ang_min),min_rad * np.sin(ang_min),max_rad * np.cos(ang_max),max_rad * np.sin(ang_max)])
            ang_min += step_small
