...
def complex_differentials(C, p):
    ...
    C : :obj:`list(int, Numpy Array, ...)`
        ...
        `C[i]` stores a :obj:`Numpy Array` matrix with the `i` simplices from `C`. 
    D : :obj:`list(Numpy Array)`
        List of differentials of `C`. The `i` entry contains a :obj:`Numpy Array` of the `i` differential for the simplicial complex. 
