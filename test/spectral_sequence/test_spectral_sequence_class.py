...
from permaviss.spectral_sequence.spectral_sequence_class import add_dictionaries


def test_double_complex():
    
    X = random_cube(500,3)
    point_cloud = take_sample(X,80)
    max_r = 0.4
    max_dim = 3
    max_div = 2
    overlap = max_r * 1.01
    ...
    for n_dim in range(MV_ss.no_columns):
        ...
        for nerv_spx in range(rk_nerve):
            for deg in range(2, MV_ss.no_rows):
                zero_coordinates = []
                coord_matrix = np.identity(len(MV_ss.subcomplexes[n_dim][nerv_spx][deg]))
                for spx_idx in range(len(MV_ss.subcomplexes[n_dim][nerv_spx][deg])):
        ...
        for n_dim in range(2, MV_ss.no_columns):
            for nerv_spx in range(len(MV_ss.nerve[n_dim])):
                ...
                if deg == 0:
                    ...
                    no_simplexes = len(MV_ss.subcomplexes[n_dim][nerv_spx][deg])
                coord_matrix = np.identity(no_simplexes)
                for spx_idx in range(no_simplexes):
                    zero_coordinates.append({nerv_spx : coord_matrix[spx_idx]})
                # end for
                cech_diff = MV_ss.cech_differential(zero_coordinates, n_dim, deg)
        ...
        for nerv_spx in range(len(MV_ss.nerve[n_dim])):
            for deg in range(1, MV_ss.no_rows):
                ...
                for i, coord in enumerate(im_left):
                    result = add_dictionaries([1,1], [coord, im_right[i]], MV_ss.p)
